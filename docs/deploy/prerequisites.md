...
* Hybrid-flash mode: there is at least one free SSD on each worker node, and the SSD capacity is larger than 60 GB.

##### Data Disk
* All-flash mode: there should be at least one free SSD on each worker node, and the SSD capacity is larger than 60 GB.
* Hybrid-flash mode: there should be at least one free HDD on each worker node, and the HDD capacity is larger than 60 GB.

#### Network Requirements
A NIC of 10GbE or above is required for the IOMesh storage network.

#### Reserved System Space
100 GB of free disk space is required in the /opt directory on each worker node for storing the IOMesh cluster metadata.
